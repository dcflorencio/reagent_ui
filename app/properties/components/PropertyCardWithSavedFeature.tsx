...
const RentalListings = ({ properties, isReload = false }: { properties: any[], isReload?: boolean }) => {
    ...
    // Method to remove property from saved properties
    const handleDeleteProperty = async (event: React.MouseEvent<HTMLButtonElement>, property: any) => {
        ...
        try {
            ...
            const response = await fetch('/api/remove_saved_property', {
                method: 'POST',
                headers: {
...
const fetchSavedProperties = async () => {
    try {
        ...
        if (response.ok) {
            ...
            console.error("Failed to fetch saved properties:", data);
            return [];
        }
    } catch (error) {
...
// const DialogHeaderCard = ({ property, isSaved, setSavedProperties }: { property: any, isSaved: boolean, setSavedProperties: (savedProperties: { property_id: string }[]) => void }) => {
//     return (
//         <Card className="relative cursor-pointer p-0 gap-2">
//             <PropertyCardHeader property={property} isSaved={isSaved} setSavedProperties={setSavedProperties} />
...
// const DialogContentCard = ({ property, showAllPhotos, setShowAllPhotos, setSavedProperties }: { property: any, showAllPhotos: boolean, setShowAllPhotos: (showAllPhotos: boolean) => void, setSavedProperties: (savedProperties: { property_id: string }[]) => void }) => {
//     return (
...
//                         <Button variant="outline" onClick={(event) => handleSaveProperty(event, property, setSavedProperties)} className="text-md font-semibold mt-2 cursor-pointer"><BookmarkIcon /></Button>
//                     </div>
//                 </div>
...
// const PropertyCardHeader = ({ property, isSaved, setSavedProperties }: { property: any, isSaved: boolean, setSavedProperties: (savedProperties: { property_id: string }[]) => void }) => {
...
//                             <Button variant="outline" className="p-3 absolute top-2 right-2" onClick={(event) => handleSaveProperty(event, property, setSavedProperties)}>
...
//                         className="w-full h-48 object-cover rounded-t-lg"
//                     />
//                     <Button variant="outline" className="p-3" onClick={(event) => handleSaveProperty(event, property, setSavedProperties)}>
//                         <Heart className={`w-6 h-6 ${isSaved ? "fill-red-500 text-red-500" : ""}`} />
//                         <span className="sr-only">Add to Favorites</span>
//                     </Button>
//                 </div>}
