                    ...
                    return (
                        <Dialog key={index}>
                            <DialogTrigger asChild>
                                <div>
                                    <DialogHeaderCard property={property} />
                                </div>
                            </DialogTrigger>
                            <DialogContent className="w-[90%] h-[90%] max-w-none max-h-none p-4">
                                ...
                                <DialogContentCard property={property} showAllPhotos={showAllPhotos} setShowAllPhotos={setShowAllPhotos} />
                            </DialogContent>
                        </Dialog>
                    );
                })}
...
const handleDeleteProperty = async (property: any) => {
    const response = await fetch('/api/delete_property', {
        method: 'DELETE',
        body: JSON.stringify({ property }),
    });
};

const DialogHeaderCard = ({ property }: { property: any }) => {
    return (
...
const DialogContentCard = ({ property, showAllPhotos, setShowAllPhotos }: { property: any, showAllPhotos: boolean, setShowAllPhotos: (showAllPhotos: boolean) => void }) => {
    return (
...
const PropertyCardHeader = ({ property }: { property: any }) => {
        ...
        <div className="relative">
            {property.carouselPhotos && property.carouselPhotos.length > 1 ?
                <Slider {...settings}>
                    {property.carouselPhotos.map((image: { url: string }, imgIndex: number) => (
                        <div key={imgIndex} className="relative">
                            ...
                            <Button variant="outline" className="p-3 absolute top-2 right-2" onClick={(event) => handleSaveProperty(event, property)}>
                                <Heart className={`w-6 h-6 ${false ? "fill-red-500 text-red-500" : ""}`} />
                ...
                <div>
                    ...
                    <Button variant="outline" className="p-3" onClick={(event) => handleSaveProperty(event, property)}>
                        <Heart className={`w-6 h-6 ${true ? "fill-red-500 text-red-500" : ""}`} />
                        <span className="sr-only">Add to Favorites</span>
                    </Button>
                </div>}
